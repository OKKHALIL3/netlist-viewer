(function(){var e=`"""Adapter: netlist_parser.Netlist -> the app's Design JSON shape.
...
`;let t=`https://cdn.jsdelivr.net/pyodide/v0.29.4/full/`,n=null;async function r(){if(!n){let r=(async()=>{let n=await(await import(`${t}pyodide.mjs`)).loadPyodide({indexURL:t});return await n.loadPackage(`micropip`),await n.pyimport(`micropip`).install(`eda-netlist-parser`),n.runPython(e),n})();r.catch(()=>{n===r&&(n=null)}),n=r}return n}let i=self;i.onmessage=async e=>{let{id:t,text:n}=e.data;try{let e=(await r()).globals.get(`parse_cdl`)(n);i.postMessage({id:t,ok:!0,json:e})}catch(e){i.postMessage({id:t,ok:!1,error:e instanceof Error?e.message:String(e)})}}})();