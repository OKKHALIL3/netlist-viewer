...
export async function layoutCell(
  ...
  const children: ElkNode[] = [
    ...
    // wherever connectivity happens to push it. Supply/ground ports are the
    // exception: they get no horizontal constraint here because they're lifted
    // onto the top/bottom rails afterward (see repositionSupplyRails).
    ...cell.ports.map(port => {
      const kind = netKindOf(port.name);
  ...
  try {
    ...
    repositionSupplyRails(cell, netKindOf, positions);
    return positions;
  } catch (err) {
...
function repositionSupplyRails(
  ...
  const powerPorts = cell.ports.filter(p => netKindOf(p.name) === 'power').map(p => portId(p.name));
  const groundPorts = cell.ports.filter(p => netKindOf(p.name) === 'ground').map(p => portId(p.name));
